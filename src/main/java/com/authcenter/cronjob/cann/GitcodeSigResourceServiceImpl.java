...
public class GitcodeSigResourceServiceImpl implements CronjobServiceInter {
    ...
    @Value("${cann.meeting.etherpad.url:}")
    private String etherpadUrl;

    @Autowired
    private CasbinServiceContext casbinServiceContext;

    @Value("${enforcers.instances.cann_sig_resource.service_name:}")
    ...
    private void syncSig() {
        LOGGER.info("sync cann gitcode sig resource start");
        try {
            ...
            LOGGER.error("parse cann gitcode sig resource failed: " + e.getMessage());
        }
        LOGGER.info("sync cann gitcode sig resource end");
    }

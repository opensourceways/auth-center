...
public class HWIDSigServiceImpl implements CronjobServiceInter {
    ...
    private void syncHwidSig() {
        LOGGER.info("sync cann huaweiID sig start");
        try {
            ...
            LOGGER.error("parse cann huaweiID sig failed: " + e.getMessage());
        }
        LOGGER.info("sync cann huaweiID sig end");
    }
    ...
    private void addUserAuth(String userName, String sig, String role) {
        try {
            ...
            LOGGER.error("add cann hwid sig-{} failed {}", userName, e.getMessage());
        }
    }

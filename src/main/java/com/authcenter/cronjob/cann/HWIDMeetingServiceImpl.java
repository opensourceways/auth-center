...
public class HWIDMeetingServiceImpl implements CronjobServiceInter {
    ...
    @Value("${enforcers.instances.cann_hwid_sig.service_name:}")
    private String casbinSigServiceName;

    @Value("${enforcers.instances.cann_hwid_meeting.service_name:}")
    ...
    private void syncMeetingAuth() {
        LOGGER.info("sync cann huaweiID meeting start");
        try {
            ...
            LOGGER.error("parse cann huaweiID meeting failed: " + e.getMessage());
        }
        LOGGER.info("sync cann huaweiID meeting end");
    }
    ...
    private void addUserAuth(String sig, String userName) {
        try {
            ...
            LOGGER.error("add cann meeting auth failed {}", e.getMessage());
        }
    }
}
